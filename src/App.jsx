...
Return ONLY a valid JSON array. Each object: {"date":"YYYY-MM-DD","description":"cleaned merchant name","amount":positive_number,"isCredit":boolean,"category":string}. Output ONLY the JSON array.`;
  ...
  const HomeContent=()=><div style={{padding:"8px 18px 24px",color:T.textPrimary}}>
    ...
    {monthlySavings.filter(m=>m.hasData).length>=2&&(()=>{
      ...
      const widths=monthlySavings.map(m=>{
        const overSpent=m.saved<0;
        // When over: bar = bills + spent (kept is dropped, spent exceeds earnings naturally)
        // When normal: bar = bills + spent + kept = earned
        return overSpent?(m.bills+m.spent):Math.max(m.earned,m.bills+m.spent+Math.max(0,m.saved));
      });
      const maxW=Math.max(...widths,1);
      return <div style={{marginBottom:22}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:10}}>
          <MicroLabel>LAST 6 MONTHS</MicroLabel>
          <div style={{display:"flex",alignItems:"center",gap:10,fontSize:9,color:T.textMuted,fontFamily:"'DM Mono'",fontWeight:600,letterSpacing:0.3}}>
            <span style={{display:"inline-flex",alignItems:"center",gap:4}}><span style={{width:8,height:8,borderRadius:4,background:T.bills}}/>BILLS</span>
            <span style={{display:"inline-flex",alignItems:"center",gap:4}}><span style={{width:8,height:8,borderRadius:4,background:T.spent}}/>SPENT</span>
            <span style={{display:"inline-flex",alignItems:"center",gap:4}}><span style={{width:8,height:8,borderRadius:4,background:T.accent}}/>KEPT</span>
          </div>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          {monthlySavings.map((m,i)=>{
            ...
            const overSpent=m.saved<0;
            const totalW=overSpent?(m.bills+m.spent):Math.max(m.earned,m.bills+m.spent+Math.max(0,m.saved));
            const widthPct=maxW>0?(totalW/maxW)*100:0;
            const billsPct=totalW>0?(m.bills/totalW)*100:0;
            const spentPct=totalW>0?(m.spent/totalW)*100:0;
            const keptPct=totalW>0&&!overSpent?(Math.max(0,m.saved)/totalW)*100:0;
            return <div key={m.month} onClick={()=>setSelectedMonth(m.month)} style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer"}}>
              <div style={{width:28,fontSize:10,color:isSel?T.accent:T.textMuted,fontWeight:isSel?700:500,fontFamily:"'DM Mono'",letterSpacing:0.3,flexShrink:0}}>{monthLabelShort(m.month).split(" ")[0].toUpperCase()}</div>
              <div style={{flex:1,height:14,display:"flex",alignItems:"center",position:"relative"}}>
                {!m.hasData
                  ?<div style={{width:"100%",height:6,background:T.borderSoft,borderRadius:3,opacity:0.5}}/>
                  :<div style={{width:`${widthPct}%`,height:14,display:"flex",gap:1,borderRadius:4,overflow:"hidden",border:isSel?`2px solid ${T.accent}`:`2px solid transparent`,boxSizing:"border-box",transition:"border-color .2s"}}>
                    {billsPct>0&&<div style={{flex:billsPct,background:T.bills}} title={`Bills: ${fmt(m.bills)}`}/>}
                    {spentPct>0&&<div style={{flex:spentPct,background:T.spent}} title={`Spent: ${fmt(m.spent)}`}/>}
                    {keptPct>0&&<div style={{flex:keptPct,background:T.accent}} title={`Kept: ${fmt(m.saved)}`}/>}
                  </div>}
              </div>
              <div style={{minWidth:60,textAlign:"right",fontSize:10,color:overSpent?T.negative:T.textSecondary,fontFamily:"'DM Mono'",fontWeight:isSel?700:500,flexShrink:0}}>
                {m.hasData?(overSpent?`-${fmt(Math.abs(m.saved))}`:fmt(m.saved)):"—"}
              </div>
            </div>;
          })}
        </div>
    ...
    {(()=>{
      ...
      const monthDIM=new Date(yr,mo,0).getDate();
      const today=new Date();
      ...
      const byDayTotal=new Array(monthDIM+1).fill(0);
      const byDayCat={};
      debits.forEach(t=>{
      ...
      const dailyCatsAvail=Object.keys(byDayCat).sort((a,b)=>byDayCat[b].reduce((s,v)=>s+v,0)-byDayCat[a].reduce((s,v)=>s+v,0));

      // Empty state — no spending transactions for this month
      if(debits.length===0){
        ...
        return <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:18,padding:"22px 18px",marginBottom:12,textAlign:"center",boxShadow:T.cardShadow}}>
          <div style={{fontSize:24,marginBottom:8}}>📊</div>
          <div style={{fontSize:13,fontWeight:600,color:T.textPrimary,marginBottom:4,fontFamily:"'Bricolage Grotesque','DM Sans',sans-serif"}}>No daily spending yet</div>
          <div style={{fontSize:11,color:T.textSecondary,lineHeight:1.6,maxWidth:280,margin:"0 auto"}}>{isCurMonthLocal?"Upload a statement or add transactions to see your daily spending line.":`No spending recorded for ${monthLabel(selectedMonth)}.`}</div>
          {monthsWithSpend.length>0&&<div style={{marginTop:12}}>
            <div style={{fontSize:9,color:T.textMuted,fontFamily:"'DM Mono'",fontWeight:600,letterSpacing:0.4,marginBottom:6}}>TRY THESE MONTHS</div>
            <div style={{display:"flex",gap:6,flexWrap:"wrap",justifyContent:"center"}}>
              {monthsWithSpend.map(m=><button key={m} onClick={()=>setSelectedMonth(m)} style={{padding:"5px 10px",borderRadius:12,border:`1px solid ${T.accent}`,background:T.accentSoft,color:T.accent,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>{monthLabelShort(m)}</button>)}
            </div>
          </div>}
        </div>;
      }
      ...
      const toggleDailyCat=cat=>setTrendCats(c=>c.includes(cat)?c.filter(x=>x!==cat):[...c,cat]);
      ...
      return <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:18,padding:"14px 16px",marginBottom:12,boxShadow:T.cardShadow}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:10}}>
          <div>
        ...
        {dailyCatsAvail.length>0&&<div style={{display:"flex",gap:6,marginBottom:12,flexWrap:"wrap",alignItems:"center"}}>
          ...
          {dailyCatsAvail.map(cat=>{
            const on=trendCats.includes(cat);
            const col=COLS[cat]||T.accent;
            return <button key={cat} onClick={()=>toggleDailyCat(cat)} style={{padding:"4px 10px",borderRadius:14,border:`1px solid ${on?col:T.border}`,background:on?col+"22":"transparent",color:on?col:T.textMuted,fontSize:11,fontWeight:on?700:500,cursor:"pointer",fontFamily:"inherit",display:"inline-flex",alignItems:"center",gap:4}}>
              <span style={{width:6,height:6,borderRadius:3,background:col,opacity:on?1:0.4}}/>
              {cat.split(" ").slice(1).join(" ")||cat}
            </button>;
          })}
        ...
        <div style={{marginTop:10,padding:"10px 12px",background:T.surface2,borderRadius:10,fontSize:11,color:T.textSecondary,lineHeight:1.5}}>
    ...
    {/* What if? — current month only (moved from Forecast modal) */}
    {isCurMonth&&dayOfMonth>5&&byCat[0]&&(()=>{
      ...
      if(remDays<=0) return null;
      return <div style={{background:T.accentSoft,border:`1px solid ${T.accentBorder}`,borderRadius:18,padding:"14px 16px",marginBottom:12}}>
    ...
    return <div>
      {!uploading&&uploadStep===0
        ...
        :<div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:20,padding:18,marginBottom:14,boxShadow:T.cardShadow}}>
          ...
          {uploadMsg&&uploadMsg.startsWith("⚠")&&<div style={{marginTop:14,padding:"12px 14px",background:T.negative+"10",border:`1px solid ${T.negative}30`,borderRadius:12,fontSize:12,color:T.negative,fontFamily:"inherit"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10}}>
              <div style={{flex:1,minWidth:0,lineHeight:1.5,wordBreak:"break-word"}}>{uploadMsg}</div>
              <button onClick={()=>{setUploadMsg("");setUploadErrDetail(null);setUploadErrExpanded(false);}} style={{background:"none",border:"none",color:T.negative,fontSize:16,cursor:"pointer",padding:"0 4px",lineHeight:1,flexShrink:0,fontFamily:"inherit"}}>×</button>
            </div>
            {uploadErrDetail&&<>
              <button onClick={()=>setUploadErrExpanded(v=>!v)} style={{marginTop:8,background:"transparent",border:`1px solid ${T.negative}40`,color:T.negative,padding:"4px 10px",borderRadius:8,fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>{uploadErrExpanded?"▾ Hide details":"▸ Show technical details"}</button>
              {uploadErrExpanded&&<div style={{marginTop:8}}>
                <div style={{padding:"8px 10px",background:T.surface,border:`1px solid ${T.borderSoft}`,borderRadius:8,fontFamily:"'DM Mono',monospace",fontSize:10,color:T.textPrimary,whiteSpace:"pre-wrap",wordBreak:"break-word",maxHeight:200,overflowY:"auto"}}>{uploadErrDetail}</div>
                <button onClick={()=>{
                  try{
                    navigator.clipboard.writeText(uploadErrDetail);
                    showToast("Copied to clipboard");
                  }catch(e){
                    showToast("Could not copy — long-press to select");
                  }
                }} style={{marginTop:6,background:"transparent",border:"none",color:T.textMuted,fontSize:10,fontWeight:500,cursor:"pointer",fontFamily:"inherit",textDecoration:"underline"}}>Copy error details</button>
                <div style={{marginTop:6,fontSize:10,color:T.textMuted,lineHeight:1.5}}>Also logged to browser console (open DevTools to see the full trace).</div>
              </div>}
            </>}
          </div>}
