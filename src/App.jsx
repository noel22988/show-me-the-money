...
function makeShareCard(month,profile,md,streams){
  ...
  ctx.fillStyle=accent; ctx.font="bold 15px Helvetica"; ctx.fillText("$how ME THE MONEY",40,46);
  ...
  ctx.fillStyle=accent+"55"; ctx.font="12px Helvetica"; ctx.fillText("$howmethemoney.app",40,470);
...
function LogoAnimation({allTimeSaved,fmt,compact}){
  const T=useTheme();
  // phases: "typing-$how" → "typing-money" → "hold-logo" → "fade-out-logo" → "counting" → "hold-number" → "fade-out-number" → repeat
  ...
  const steps=[
    ...
    <div key="s0">
      <div style={{textAlign:"center",marginBottom:28}}>
        ...
        <div style={{fontSize:32,fontWeight:700,color:T.textPrimary,letterSpacing:-0.5,lineHeight:1.2}}>$how Me The Money</div>
...
export default function App(){
  ...
  const [$howReset,set$howReset]=useState(false);
  const [$howPrivacy,set$howPrivacy]=useState(false);
  ...
  const archiveTx=id=>{ const tx=(md.txs||[]).find(t=>t.id===id); if(!tx) return; saveArchive([{...tx,archivedAt:new Date().toISOString()},...archive].slice(0,500)); saveMD(selectedMonth,{txs:(md.txs||[]).filter(t=>t.id!==id)}); $howToast("Moved to archive"); };
  const restoreToReview=id=>{ const tx=archive.find(t=>t.id===id); if(!tx) return; const{archivedAt:_,...clean}=tx; setPendingTxs(p=>[{...clean,checked:true,source:clean.source||"manual"},...p]); saveArchive(archive.filter(t=>t.id!==id)); $howToast("✓ Moved to Review"); setTab("review"); };
  ...
  const commitTransactions=async()=>{
    ...
    $howToast(`✓ ${added} transactions saved — ${months.length>1?`${monthLabel(months[0])} – ${monthLabel(months[months.length-1])}`:monthLabel(months[0]||selectedMonth)}`);
  ...
  const editTx=async draft=>{ await saveMD(selectedMonth,{txs:(md.txs||[]).map(t=>t.id===draft.id?draft:t)}); $howToast("Updated"); };
  ...
  const addManual=async({description,amount,category,date}={})=>{
    ...
    if(profile?.startMonth&&month<profile.startMonth){ $howToast("⚠ Before your start month"); return; }
    const ex=(monthlyData[month]||{}).txs||[];
    await saveMD(month,{txs:[tx,...ex]});
    $howToast("Transaction added");
...
Return ONLY a valid JSON array. Each object: {"date":"YYYY-MM-DD","description":"cleaned merchant name","amount":positive_number,"isCredit":boolean,"category":string}. Output ONLY the JSON array.`;
  ...
  const handleFixedCommitConfirm=confirmed=>{
    ...
    if(toAdd.length>0){saveProfile({...profile,fixedCommitments:[...existing,...toAdd]});$howToast(`✓ Added ${toAdd.length} fixed commitment${toAdd.length>1?"s":""}`);}
  ...
  const MoneyContent=()=><div style={{display:"flex",flexDirection:"column",gap:16}}>
    ...
    <Card>
      ...
      {monthFixed.length===0?<p style={{fontSize:14,color:T.textMuted,margin:0}}>Add in Profile →</p>:<div>
        {monthFixed.map(c=><div key={c.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:`1px solid ${T.border}`}}>
          ...
          <EditableAmt value={+c.amount||0} fmt={fmt} color={T.info} onSave={v=>{saveProfile({...profile,fixedCommitments:profile.fixedCommitments.map(x=>x.id===c.id?{...x,amount:v}:x)});$howToast("Updated");}}/>
        </div>)}
    ...
    <Card>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
        ...
        {(Object.keys(mf).length>0||Object.keys(cf).length>0)&&<button onClick={()=>{setEh({});setCh({});lsSave("excludeHistory",{});lsSave("catExcludeHistory",{});$howToast("Habits cleared");}} style={{padding:"4px 12px",background:"transparent",border:`1px solid ${T.border}`,borderRadius:8,color:T.negative,fontSize:12,cursor:"pointer",fontFamily:"inherit"}}>Clear all</button>}
      </div>
...
// ╚══════════════════════════════════════════════════════════════╝
  ...
  const ProfileContent=()=>{
    ...
    const [$howRst,set$howRst]=useState(false);
    ...
    const handleRestFile=e=>{
      ...
      r.onload=()=>{
        try{
          ...
          setRestCand(snap);
        }catch{ $howToast("⚠ Invalid backup file"); }
      };
    ...
    const doRest=snap=>{
      ...
      setRestCand(null); $howToast("✓ Backup restored"); window.location.reload();
    };
    ...
    return <div style={{display:"flex",flexDirection:"column",gap:16,paddingBottom:40}}>
      ...
      {$howRst&&<ResetModal onConfirm={doReset} onClose={()=>set$howRst(false)} onDownloadFirst={()=>{dlBackup(profile,monthlyData,eh,ch,insights,archive);$howToast("Backup downloaded");}}/>}
      ...
      <Card>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          ...
          <div style={{display:"flex",gap:10}}>
            <button onClick={()=>set$howPrivacy(true)} style={{flex:1,padding:"13px 16px",background:"transparent",border:`1px solid ${T.border}`,borderRadius:11,color:T.textSecondary,fontFamily:"inherit",fontSize:14,cursor:"pointer",textAlign:"left",display:"flex",alignItems:"center",gap:10}}>
      ...
      {/* ── Reset ── */}
      <button onClick={()=>set$howRst(true)} style={{padding:"13px",background:"transparent",border:`1px solid ${T.negative}40`,borderRadius:12,fontFamily:"inherit",fontSize:14,color:T.negative,cursor:"pointer",width:"100%",marginBottom:8}}>Reset Everything & Start Again</button>
    </div>;
  };
  // ── renderContent ────────────────────────────────────────────────────────────
  const renderContent=()=>{
  ...
  return <ThemeCtx.Provider value={theme}>
    <div style={{minHeight:"100vh",background:T.bg,color:T.textPrimary,fontFamily:"'DM Sans','Helvetica Neue',sans-serif",display:"flex",flexDirection:isDesktop?"row":"column"}}>
      ...
      {$howPrivacy&&<PrivacyModal onClose={()=>set$howPrivacy(false)}/>}
      ...
      {$howReset&&<ResetModal onConfirm={doReset} onClose={()=>set$howReset(false)} onDownloadFirst={()=>{dlBackup(profile,monthlyData,eh,ch,insights,archive);$howToast("Backup downloaded");}}/>}
      ...
      {isDesktop&&<div style={{width:SIDEBAR_W,background:T.surface,borderRight:`1px solid ${T.border}`,display:"flex",flexDirection:"column",padding:"28px 14px",position:"fixed",top:0,left:0,bottom:0,zIndex:50,boxShadow:`4px 0 24px rgba(0,0,0,${T.bgLight?0.05:0.18})`}}>
        ...
        <div style={{display:"flex",gap:14,padding:"14px 6px 0",borderTop:`1px solid ${T.border}`,marginTop:12}}>
          <button onClick={()=>set$howPrivacy(true)} style={{background:"none",border:"none",color:T.textMuted,fontSize:11,cursor:"pointer",fontFamily:"inherit",padding:0}}>🔒 Privacy</button>
